...
//注意这个{User}的写法
const {Changyongzhuangbei} = require('../model/mode')

//获取列表
router.get('/',async (req,res)=>{
  const list = await Changyongzhuangbei.find()
  res.send(list)
})
...
router.post('/register',async (req,res)=>{
  ...
  const newYiliangsan = await new Changyongzhuangbei(addData).save()
...
router.post('/delete',async (req,res)=>{
  const deleteUser = await Changyongzhuangbei.deleteOne({_id:req.body._id})
  if(deleteUser){
    res.send(deleteUser)
  }
})
...
router.post('/edit',async (req,res)=>{
  ...
  const oldguanliyuan = await Changyongzhuangbei.findOne({_id:req.body._id})
  console.log(oldguanliyuan)
  var newuser = await Changyongzhuangbei.findOne({_id:req.body._id})
  ...
  console.log(newuser)
  const editUser =await Changyongzhuangbei.updateOne(oldguanliyuan,{$set:newuser})
  if(editUser){
    res.send(editUser)
  }
})
